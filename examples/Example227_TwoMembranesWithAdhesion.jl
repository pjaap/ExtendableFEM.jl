...
function make_kernel(f1, # membrane force 1 
	...
	function nonlinear_kernel!(result, input, qpinfo )
		...
		result[1]   =  γ*max(0, u1 - u2 )- f1 #  - θ*( -d_χ(u2 - u1, ϵ) )
		result[2:3] = ∇u1												             

		# second membrane force 
		result[4]   =   -γ*max(0, u1 - u2 ) - f2 #	 + θ*( -d_χ(u2 - u1, ϵ) )     	     
		result[5:6] = ∇u2												             
...
function main(;
	...
	nonlinear_kernel! = make_kernel(f1,f2,θ,γ,ϵ,boundary_offset)
	assign_operator!(PD, NonlinearOperator(nonlinear_kernel!, [id(u1), grad(u1), id(u2), grad(u2)]; bonus_quadorder=2, sparse_jacobians=false, kwargs...))
	...
	@show minimum(view(sol[u2])) maximum(view(sol[u1]))
	...
	@show minimum(abs.(view(sol[u1]) .- view(sol[u2])))
...
Example227_TwoMembranesWithAdhesion.main(Plotter=GLMakie, γ=1e8, θ=0, ϵ=1e-1, N=30, f1 = 25, f2 = -0)